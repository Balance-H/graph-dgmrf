...
class FlexLayer(LinearLayer):
    def __init__(self, graph, config, vi_layer=False):
        ...
        self.graph = graph
        self.config = config
        self._logdet_cache_iter = None
        self._logdet_cache_val = None
        self._logdet_calls = 0
        #heng
        ...
        if self.dist_weighted:
            ...
            #heng
            elif config["log_det_method"] == "decomp":  
                pass
            #heng
            else:
    ...
    def log_det(self):
        ...
        if self.config["log_det_method"] == "decomp":
            # ---- cache within one iteration ----
            it = self.config.get("_iter", None)
            if it is not None and self._logdet_cache_iter == it and self._logdet_cache_val is not None:
                return self._logdet_cache_val
            
            self._logdet_calls += 1
            if self._logdet_calls <= 5:
                print(f"[debug] log_det compute at iter={it}")

            val = utils.decomp_logdet_schur_cached(
                graph=self.graph,
                degrees=self.degrees,
                a1=self.self_weight[0],
                a2=self.neighbor_weight[0],
                gamma=self.degree_power,
            )

            if it is not None:
                self._logdet_cache_iter = it
                self._logdet_cache_val = val
            return val
